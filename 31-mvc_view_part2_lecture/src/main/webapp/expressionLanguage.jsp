...
<body>
	...
	<p>Every JSP EL variable is stored in one of four scopes: page,
		request, session, or application. We will only use the first three.</p>
	...
	<p>Identifiers can also be used within JSP EL expressions without
		specifying a scope. In this case, all of the scopes are searched for
		the identifier</p>
	...
	<p>When searching the various scopes for an identifier, they are
	...
	<c:set var="ambiguousReference"
		value="ambiguousReference is in session scope" scope="session" />
	<c:set var="ambiguousReference"
		value="ambiguousReference is in request scope" scope="request" />

	<p>So, for instance, if an identifier appears in both requestScope
		and sessionScope, the requestScope identifier takes precedence when
		the identifier is referenced without a scope qualifier</p>
	...
	<p>Array elements are referenced in JSP EL in the same way as Java.
		However, one difference is that Lists can also be referenced using the
		same syntax as arrays</p>
	<ul>
		<li><em>stringList[2]</em> is <b>${stringList[2]}</b></li>
	</ul>

	<p>Map elements can be accessed using the same syntax as JavaBeans
		object attributes. The key name is used after the dot operator in the
		same way a property name is used with a JavaBean.</p>
	...
	<p>
		The Mustang is made by <b>${model2Manufacturer.Mustang}</b>.
	</p>
	<p>
		The Camry is made by <b>${model2Manufacturer.Camry}</b>.
	</p>
