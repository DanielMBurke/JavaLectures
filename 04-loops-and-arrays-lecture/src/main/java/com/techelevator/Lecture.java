...
public class Lecture {
    ...
    */
    public int[] returnArray() {
        int[] array = {80,8080,443}; //initializes array to 3 elements (80, 8080 AND 443)
        return array;
    }
    ...
    */
    public int returnVariableFromBlock(int number) {  
    	int result = number * 5; // this variable result is local to this block
        
        { // A new block with scoped variables - variable result created

        } // the result variable disappears here

        return result; // We want to return result here. How? Move the variable outside the defined block.
    }
    ...
    */
	public boolean returnOperationInBlock() {
		...
		return (result == 50); // 50 <-- Change the number to match result and make this be true
	}
    ...
    */
    public boolean returnCounterFromLoop() {
        ...
        return (counter == 4); // What should the number be to return true? 4 times (1 for each element 3,4,2,9)
    }
    ...
    */
	public boolean returnCorrectCount() {
		...
		int counter = 0;
//int i ** can also be defined outside the for statement (loop) in case you need to reference it outside the loop scope **
